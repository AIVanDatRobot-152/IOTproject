...
// Điều khiển LED2 (Blynk V1)
BLYNK_WRITE(V1) {
  int value = param.asInt();
  Serial.println("LED2: " + String(value));
  value ? digitalWrite(LED2, HIGH) : digitalWrite(LED2, LOW);
}

// Điều khiển LED3 (Blynk V0)
BLYNK_WRITE(V0) {
  int value = param.asInt();
  Serial.println("LED3: " + String(value));
  value ? digitalWrite(LED3, HIGH) : digitalWrite(LED3, LOW);
}
